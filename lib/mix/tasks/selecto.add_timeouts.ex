defmodule Mix.Tasks.Selecto.AddTimeouts do
  ...
  defp generate_monitor_code(options, app_module) do
    ...
    defmodule #{inspect(Module.concat(app_module, QueryTimeoutMonitor))} do
      @moduledoc \"\"\"
      Monitors database query performance and implements a circuit breaker.

      This generated monitor is standalone and does not depend on an external
      `Selecto.QueryTimeoutMonitor` module.
      ...
      def allow_query?, do: GenServer.call(__MODULE__, :allow_query?)
      def circuit_state, do: GenServer.call(__MODULE__, :circuit_state)
      def stats, do: GenServer.call(__MODULE__, :stats)
      def record_query(duration_ms), do: GenServer.cast(__MODULE__, {:record_query, duration_ms})
      def record_timeout, do: GenServer.cast(__MODULE__, :record_timeout)
      def record_slow_query(duration_ms), do: GenServer.cast(__MODULE__, {:record_slow_query, duration_ms})

      @impl true
      def init(_opts) do
        schedule_check()
        {:ok, %__MODULE__{pool_size: repo_pool_size()}}
      end

      @impl true
      def handle_call(:allow_query?, _from, state) do
        {:reply, state.circuit_state != :open, state}
      end
        ...
        state =
          cond do
            duration_ms >= @very_slow_query_threshold ->
              %{state | very_slow_queries: state.very_slow_queries + 1}
      ...
      def handle_cast(:record_timeout, state) do
        {:noreply,
         %{state | timeouts: state.timeouts + 1, last_timeout_at: DateTime.utc_now() |> DateTime.truncate(:second)}}
      end

      def handle_cast({:record_slow_query, duration_ms}, state) do
        handle_cast({:record_query, duration_ms}, state)
      end

      @impl true
      def handle_info(:check_circuit, state) do
        state = maybe_advance_circuit(state)
        schedule_check()
        {:noreply, state}
      end

      defp maybe_advance_circuit(%{circuit_state: :open, circuit_opened_at: opened_at} = state) do
        if opened_at && DateTime.diff(DateTime.utc_now(), opened_at, :millisecond) >= @circuit_half_open_timeout do
      ...
      defp maybe_advance_circuit(state), do: state

      defp schedule_check do
        Process.send_after(self(), :check_circuit, @check_interval)
      end

      defp repo_pool_size do
        @repo.config()[:pool_size] || 10
      rescue
        _ -> 10
      end
    end
