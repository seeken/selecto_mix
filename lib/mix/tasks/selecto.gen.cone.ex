defmodule Mix.Tasks.Selecto.Gen.Cone do
  @moduledoc """
  Generates a SelectoCone LiveView .exs file for hierarchical data management.
  ...
  ## Usage
      ...
      mix selecto.gen.cone Customer Rental Payment [options]
  ...
  ## Options
    ...
    * `--output` - Output file path (default: selecto_cone_[apex].exs)
    * `--port` - Port for the Phoenix server (default: 4090)
    * `--route` - Route path (default: /selecto_cone)
  ...
  ## Examples
      ...
      mix selecto.gen.cone Customer Rental Payment

      # Two-level hierarchy
      mix selecto.gen.cone Order OrderItem

      # Custom output and port
      mix selecto.gen.cone Customer Rental Payment --output my_cone.exs --port 5000

      # Read-only view without forms
      mix selecto.gen.cone Customer Rental Payment --no-forms
  """

  use Mix.Task
  import Mix.Generator

  @shortdoc "Generates a SelectoCone LiveView .exs file"
  ...
  defp build_config(schema_names, opts) do
    ...
    %{
      ...
      output: Keyword.get(opts, :output, "selecto_cone_#{Macro.underscore(apex)}.exs"),
      port: Keyword.get(opts, :port, 4090),
      route: Keyword.get(opts, :route, "/selecto_cone"),
  ...
  defp generate_cone(config) do
    Mix.shell().info("Generating SelectoCone LiveView...")
    ...
    Then visit:
        http://localhost:#{config.port}#{config.route}
  ...
  defp build_template(config) do
    ...
    # SelectoCone LiveView - Generated #{Date.to_string(Date.utc_today())}
    # 
    ...
    defmodule SelectoCone.Router do
      ...
      scope "/" do
        ...
        live "#{config.route}", SelectoCone.ConeLive
      end
    end
      ...
      def root(assigns) do
        ...
        <html lang="en">
          <head>
            ...
            <title>SelectoCone - #{config.apex.human} Management</title>
    ...
    📍 Visit: http://localhost:#{config.port}#{config.route}
  ...
  defp generate_event_handlers(config) do
    ...
    # Event Handlers
      ...
      def handle_event("select_#{config.apex.singular}", %{"id" => id}, socket) do
        ...
        {:noreply, push_patch(socket, to: "#{config.route}?#{config.apex.singular}_id=\#{id}")}
      end
  ...
  defp generate_middle_handlers(config) do
    ...
    def handle_event("select_#{config.middle.singular}", %{"id" => id}, socket) do
        ...
        {:noreply, push_patch(socket, to: "#{config.route}?#{config.apex.singular}_id=\#{#{config.apex.singular}_id}&#{config.middle.singular}_id=\#{id}")}
      end
    """
  end
